                ...
                if (ExistPrinterCOnfig == true)
                {
                    if (contadorTIcket == CantidadLoterias)
                    {
                       ...
                       TicketTemplateHelper.PrintTicket(ImprimirTicket, listaConfiguraciones);*/
                        ...
                        TicketValue ticketEnviar = new TicketValue() { BanNombre = Autenticador.BancaConfiguracion.BancaDto.BanNombre, Direccion = Autenticador.BancaConfiguracion.BancaDto.BanDireccion, FechaActual = MarBetResponse.StrFecha, Telefono = Autenticador.BancaConfiguracion.BancaDto.BanTelefono, Jugadas = jugadasTicket, LoteriaTicketPin = loteriatickpin, Firma = firma, Texto = null, Total = jugadasTicket.Sum(x => x.Jugada.Monto).ToString("N2"), AutorizacionHacienda = null, Logo = null };
