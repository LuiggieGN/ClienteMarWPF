                ...
                switch (item.ConfigKey)
                {
                    ...
                    case "TicketsGanadores":
                        ...
                        if (TicketPagados.Count() > 0)
                        {
                            ...
                            {
                                WriteTextColumn(g, new List<string> { pagados.TicketNo, FechaHelper.FormatFecha(Convert.ToDateTime(pagados.StrFecha), FechaHelper.FormatoEnum.FechaCorta), pagados.StrHora.ToString(), pagados.Pago.ToString("C2") }, FontSize - 1, FontStyle.Regular.ToString().ToLower(), AlignamenCenter);
                            }
                            WriteLineFinas(g);
                        ...
                        if (TicketPendientePagos.Count() > 0)
                        {
                            ...
                            {
                                WriteTextColumn(g, new List<string> { pendientesPagos.TicketNo, FechaHelper.FormatFecha(Convert.ToDateTime(pendientesPagos.StrFecha), FechaHelper.FormatoEnum.FechaCorta), pendientesPagos.StrHora, pendientesPagos.Pago.ToString("C2") }, FontSize - 1, FontStyle.Regular.ToString().ToLower(), AlignamenCenter);
                            }
                            WriteLineFinas(g);
