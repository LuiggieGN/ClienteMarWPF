...
{
    public class SorteosMarIndexLogic
    {
        ...
        public static object GetProductosDisponibles()
        {
            ...
            productoRef.Add(DataAccess.Tables.Enums.DbEnums.VP_ProductoReferencia.CincoMinutos);
            reciboRef.Add(DataAccess.Tables.Enums.DbEnums.ReciboCampoReferencia.CincoMinutos);
        ...
        public static object GetTicket_VendidosHoy(int pRiferoId, int pBancaId, DbEnums.Productos pProductoLogicaKey)
        {
            DateTime desde = DateTime.Today;
            DateTime hasta = DateTime.Today.AddDays(1);
            try
            {
                var cuentaId = DataAccess.EFRepositories.VpCuentaRepository.GetVpCuentaNew(pRiferoId, pProductoLogicaKey).CuentaID;
                ...
                var tickets = JuegaMasViewModel.MapFromTransacciones(transacciones,false,false).OrderByDescending(x => x.TicketID);
                if (tickets.Any())
                {
                    return new { OK = true, Tickets = tickets };
                }
                else
                {
                    return new { OK = true, Tickets = string.Empty };
                }
        ...
        public static object GetUltimosSorteos(int pBancaId, int pRiferoId)
        {
            return new {OK = true, Sorteos = ""};
        }
