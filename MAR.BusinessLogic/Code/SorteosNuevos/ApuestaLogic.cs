...
namespace MAR.BusinessLogic.Code.SorteosNuevos
{
    ...
    {
        public static string Apuesta(string pTicketId)
        {
            ...
            var cuenta = DataAccess.EFRepositories.PMCuentasRepository.GetPMCuenta(100000, "Hacienda", 0,1);
            ...
            {
                jugadas.Add(new Juego { Codigo = item.Codigo, Jugada = ApuestaHelper.SeparaJugadaConGuion(item.Jugada), Monto = item.Monto, TipoJugadaID = item.TipoJugadaID, });
            }
            ...
            {
                //Consume servicio
                var apuesta = ClienteHTTP.CallService<ApuestaResponseModel, ApuestaRequestModel>(MetodosEnum.MetodoServicio.Apuesta, p, MetodosEnum.HttpMethod.POST, autenticacion);
