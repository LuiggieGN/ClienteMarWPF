...
{
    public class UltimoCommand : ActionCommand
    {
        private readonly ConsultaMovimientoViewModel _viewmodel;
        private readonly ICajaService _cajaService;

        public UltimoCommand(ConsultaMovimientoViewModel viewmodel,
                             ICajaService cajaService) : base()
        {
            SetAction(new Action<object>(MoveLastPage),
                      new Predicate<object>(CanMoveLastPage));

            _viewmodel = viewmodel;
            _cajaService = cajaService;
        }

        public bool CanMoveLastPage(object parameter)
        {
            return  _viewmodel.Movimientos.PaginaNo != _viewmodel.Movimientos.TotalPaginas;
        }

        public void MoveLastPage(object parametro)
        {
            try
            {
                if (
                   ...
                   )
                {
                    ...
                    toSend.PaginaNo = _viewmodel.Movimientos.TotalPaginas;
